...
const CharList = ({showCharInfo}) => {
    ...
    const getAllCharacters = async (limit = 9, offset = 0) => {
        ...
        setCharList(
            resuts.map(pokemon => {
                return(
                    <li key={pokemon.id} className={classList} onClick={() => {showCharInfo(pokemon.id)}} >
                        <img src={`${pokemon.sprites.other.dream_world.front_default}`} alt={pokemon.name}  onMouseUp = {() => { classList += '_selected'}}/>
